...
struct GitHubCommitCard: View {
    ...
    private var goal: Int { habit.commitTarget ?? 1 }
    private var met: Bool { commits >= goal }
    
    // Check if habit is verified/completed today
    private var isCompletedToday: Bool {
        habitManager.verifiedHabitsToday[habit.id] == true || met
    }
    ...
    var body: some View {
        ZStack(alignment: .topTrailing) {
            VStack(spacing: 0) {
                ...
                // Large centered icon (GitHub logo)
                Image("github")
                    .resizable()
                    .scaledToFit()
                ...
                VStack(spacing: 8) {
                    ...
                    GeometryReader { geometry in
                        ZStack(alignment: .leading) {
                            ...
                            RoundedRectangle(cornerRadius: 6)
                                .fill(Color.white)
                                .frame(width: geometry.size.width * min(Double(commits) / Double(goal), 1.0), height: 6)
                                .animation(.easeInOut(duration: 0.8), value: commits)
                        }
                    }
                    .frame(height: 6)
                    
                    // Current/Target display
                    Text("\(commits) / \(goal) commit\(goal == 1 ? "" : "s")")
                        .font(.custom("EB Garamond", size: cardWidth * 0.055))
                        .foregroundColor(.white)
                        .multilineTextAlignment(.center)
                }
                ...
                HStack {
                    ...
                    HStack(spacing: cardWidth * 0.01) {
                        if isCompletedToday {
                            ...
                            let progressPercentage = min(100, Int((Double(commits) / Double(goal)) * 100))
